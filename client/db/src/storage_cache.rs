...
pub struct CachingState<S, B: BlockT> {
	...
	overlay_stats: sp_runtime_interface::stats::StateMachineStats,
	/// Backing state.
	state: S,
	/// Cache data.
...
impl<S: StateBackend<HashFor<B>>, B: BlockT> CachingState<S, B> {
	...
	pub(crate) fn new(
		...
		CachingState {
			...
			overlay_stats: sp_runtime_interface::stats::StateMachineStats::default(),
			state,
			cache: CacheChanges {
				shared_cache,
...
impl<S: StateBackend<HashFor<B>>, B: BlockT> StateBackend<HashFor<B>> for CachingState<S, B> {
	...
	fn register_overlay_stats(&mut self, stats: &sp_runtime_interface::stats::StateMachineStats) {
		self.overlay_stats.add(stats);
	}

	fn usage_info(&self) -> sp_runtime_interface::stats::UsageInfo {
