...
use sc_client_api::{
	ClientBackend, BlockchainEvents, CallExecutor,
	backend::{AuxStore, Backend}, ExecutionStrategy, Finalizer
};
...
pub trait ClientForGrandpa<Block, BE>:
	ClientBackend<Block, BE> + AuxStore + HeaderMetadata<Block, Error = sp_blockchain::Error>
	+ HeaderBackend<Block> + BlockchainEvents<Block> + Finalizer<Block, BE> + ProvideRuntimeApi<Block>
	where
		BE: Backend<Block>,
		Block: BlockT,
{}
